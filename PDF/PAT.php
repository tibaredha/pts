...
{ 
	...
	$h=155;
	$this->SetXY(05,$h-5); 	  
	$this->cell(200,05,"List Consultations",1,0,'C',0);
	$this->SetXY(05,$h); 	  
	$this->cell(20,05,"Date Con",1,0,'C',0);
	$this->SetXY(25,$h); 	  
	$this->cell(22,05,"Lieu du Con ",1,0,'C',0);
	$this->SetXY(47,$h); 	  
	$this->cell(44,05,"N° Con ",1,0,'C',0);
	$this->SetXY(91,$h); 	  
	$this->cell(20,05,"TA",1,0,'C',0);
	$this->SetXY(111,$h); 	  
	$this->cell(20,05,"Poids",1,0,'C',0);
	$this->SetXY(131,$h); 	  
	$this->cell(30,05,"Diagnostic",1,0,'C',0);
	$this->SetXY(161,$h); 	  
	...
	{
	$this->entetepage("FICHE D'EXAMEN");
	$this->mysqlconnect();
	$query1 = "select * from PAT WHERE  id = '$IDDNR'";
	$resultat1=mysql_query($query1);
	$row = mysql_fetch_array($resultat1); // $IDDNR=$row['IDDNR'];
	$this->SetFont('Arial','I', 13);
	$this->Text(5,48,"IDENTIFICATION DU MALADE  :"); 
...
//***************************************************RESULTAT DON **********************************************************************************//
	...
	function RSS($id,$IDDNR)
    {
		...
		{
		   $this->SetTextColor(225,0,0);
		    $this->Text(64,88,$result1->DATEDON);//DATE D ADMISSION A L'HOPITAL:
			$this->Text(58,106,$result1->DATEDON);//DATE D ENTREE AU SERVICE:
			...
			while($result2=mysql_fetch_object($resultat2))
			{
				...
				$this->Text(154,25,$this->dateUS2FR($result2->DATENAISSANCE));
			...
			while($result3=mysql_fetch_object($resultat3))
			{
			   $this->SetTextColor(225,0,0);
				...
				$this->Text(50,45,$result3->DATEDON);//DATE HOSPITALISATION
				$this->Text(22,55,$this->nbrtostring("mvc","service","ids",$result3->SERVICE,"servicefr"));// 
				$this->Text(114,55,$result3->DATEDON);//"DATE D'ENTREE AU SERVICE:".
